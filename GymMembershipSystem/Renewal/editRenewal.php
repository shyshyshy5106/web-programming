...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty(array_filter($errors))) {
        ...
        if($renewalObj->editRenewal($_GET["renewal_id"])){
...
<head>
    ...
    <title>Edit Renewal Record</title>
    <link rel="stylesheet" href="../index.css">
</head>
<body>
   <main class="page-container">
        <selection class="form-card">
            <h1>Edit Renewal Record</h1>
                        ...
                        <select class="form-control" name="membership_id" id="membership_id">
                            ...
                            <?php if (!empty($memberships)) : ?>
                                <?php foreach ($memberships as $ms) : ?>
                                    ...
                                    <option value="<?= $msId ?>" <?= (isset($renewal['membership_id']) && $renewal['membership_id'] == $msId) ? 'selected' : '' ?>>Membership <?= $msId ?> - <?= htmlspecialchars($mName) ?> - Plan <?= $planId ?></option>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </select>
                        ...
                        <select class="form-control" name="plan_id" id="plan_id">
                            ...
                            <?php if (!empty($plans)) : ?>
                                <?php foreach ($plans as $p) : ?>
                                    ...
                                    <option value="<?= $pId ?>" <?= (isset($renewal['plan_id']) && $renewal['plan_id'] == $pId) ? 'selected' : '' ?>><?= htmlspecialchars($pName) ?> (ID: <?= $pId ?>)</option>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </select>
            ...
            <div class="form-group">
                <label for="renewal_date">Renewal Date <span class="required">*</span></label>
                    <input class="form-control" type="date" name="renewal_date" id="renewal_date" value="<?=$renewal["renewal_date"] ?? "" ?>">
                    <p class="error"><?=$errors["renewal_date"] ?? "" ?></p>
        </div>

            <div class="form-group">
                <label for="previous_start_date">Previous Start Date<span>*</span></label>
                    <input class="form-control" type="date" name="previous_start_date" id="previous_start_date" value="<?=$renewal["previous_start_date"] ?? "" ?>">
            ...
            <div class="form-group">
                <label for="previous_expiry_date">Previous Expiry Date<span class="required">*</span></label>
                    <input class="form-control" type="date" name="previous_expiry_date" id="previous_expiry_date" value="<?=$renewal["previous_expiry_date"] ?? "" ?>">
                    <p class="error"><?=$errors["previous_expiry_date"] ?? "" ?></p>
        </div>

            <div class="form-group">
                <label for="new_start_date">New Start Date <span class="required">*</span></label>
                    <input class="form-control" type="date" name="new_start_date" id="new_start_date" value="<?=$renewal["new_start_date"] ?? "" ?>">
                    <p class="error"><?=$errors["new_start_date"] ?? "" ?></p>
        </div>

            <div class="form-group">
                <label for="new_expiry_date">New Expiry Date <span class="required">*</span></label>
                    <input class="form-control" type="date" name="new_expiry_date" id="new_expiry_date" value="<?=$renewal["new_expiry_date"] ?? "" ?>">
                        ...
                        <select class="form-control" name="payment_id" id="payment_id">
                            ...
                            <?php if (!empty($payments)) : ?>
                                <?php foreach ($payments as $pay) : ?>
                                    <?php $payId = $pay['payment_id'] ?? $pay['id'] ?? null; $payDate = $pay['payment_date'] ?? ''; $payAmt = $pay['amount'] ?? ''; $payMs = $pay['membership_id'] ?? ''; ?>
                                    <option value="<?= $payId ?>" <?= (isset($renewal['payment_id']) && $renewal['payment_id'] == $payId) ? 'selected' : '' ?>>Payment <?= $payId ?> - <?= htmlspecialchars($payDate) ?> - <?= htmlspecialchars($payAmt) ?> (Membership <?= $payMs ?>)</option>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </select>
                        ...
                        <select class="form-control" name="employee_id" id="employee_id">
                            ...
                            <?php if (!empty($employees)) : ?>
                                <?php foreach ($employees as $e) : ?>
                                    <?php $eId = $e['id']; $eName = trim($e['fname'] . ' ' . ($e['mname'] ? $e['mname'] . ' ' : '') . $e['lname']); ?>
                                    <option value="<?= $eId ?>" <?= (isset($renewal['employee_id']) && $renewal['employee_id'] == $eId) ? 'selected' : '' ?>><?= htmlspecialchars($eName) ?> (ID: <?= $eId ?>)</option>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </select>
                        <p class="error"><?= $errors['employee_id'] ?? '' ?></p><br><br>
            </div>

            <div class="footer-actions">
                <input type="submit" class="btn btn-primary" value="Update Renewal"> <span><button><a class="btn btn-outline" href="viewRenewal.php">Cancel</a></button></span>
                </div>
            </form>
        </selection>
    </main>
