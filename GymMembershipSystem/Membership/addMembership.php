<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if (!empty($membership["member_id"]) && !empty($membership["plan_id"]) && 
        $membershipObj->isMembershipExist($membership["member_id"], $membership["plan_id"])) {
        ...
        if (empty(array_filter($errors))) {
            ...
            if ($membershipObj->addMembership()) {
                header("Location: viewMembership.php");
            } else {
                echo "Failed to add membership. Please try again.";
            }
        }
    }
?>
...
<div class="page-container">
    <div class="form-card">
        ...
        <form action="" method="POST">
                ...
                <select class="form-control" name="member_id" id="member_id">
                    ...
                    <?php if (!empty($members)) : ?>
                        <?php foreach ($members as $m) : ?>
                            <?php
                                $mId = $m['id'];
                                $mName = trim($m['fname'] . ' ' . ($m['mname'] ? $m['mname'] . ' ' : '') . $m['lname']);
                            ?>
                            <option value="<?= $mId ?>" <?= (isset($membership['member_id']) && $membership['member_id'] == $mId) ? 'selected' : '' ?>>
                                <?= htmlspecialchars($mName) ?> (ID: <?= $mId ?>)
                            </option>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </select>
                ...
                <select class="form-control" name="plan_id" id="plan_id">
                    ...
                    <?php if (!empty($plans)) : ?>
                        <?php foreach ($plans as $p) : ?>
                            ...
                            ?>
                            <option value="<?= $pId ?>" <?= (isset($membership['plan_id']) && $membership['plan_id'] == $pId) ? 'selected' : '' ?>>
            ...
            <div class="form-group">
                <label for="original_expiry_date">Original Expiry Date <span class="required">*</span></label>
                <input class="form-control" type="date" name="original_expiry_date" id="original_expiry_date"
                    value="<?=$membership["original_expiry_date"] ?? "" ?>">
                ...
                <select class="form-control" name="employee_id" id="employee_id">
                    ...
                    <?php if (!empty($employees)) : ?>
                        <?php foreach ($employees as $e) : ?>
                            ...
                            ?>
                            <option value="<?= $eId ?>" <?= (isset($membership['employee_id']) && $membership['employee_id'] == $eId) ? 'selected' : '' ?>>
            ...
            <div class="footer-actions">
                <input class="btn btn-primary" type="submit" value="Save Membership">
                <a class="btn btn-outline" href="viewMembership.php">View Memberships</a>
            </div>
