...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (!empty($payment["membership_id"]) && !empty($payment["payment_date"]) &&
        $paymentObj->isPaymentExist($payment["membership_id"], $payment["payment_date"])) {
        $errors["membership_id"] = "A payment for this membership on the selected date already exists.";
    }
    ...
    if (empty(array_filter($errors))) {
        ...
        if ($paymentObj->addPayment()) {
            header("Location: viewPayment.php");
        } else {
            echo "Failed to add payment. Please try again.";
        }
    }
}
?>
    ...
    <main class="page-container">
        <section class="form-card">
                ...
                <div class="footer-actions">
                    <input type="submit" class="btn btn-primary" value="Save Payment"> <span><button><a class="btn btn-outline" href="viewPayment.php">Cancel</a></button></span>
                </form>
        </section>
    </main>
</body>
